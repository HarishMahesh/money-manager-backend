...
const createNewExpense = async (req, res) => {
  let { description, type, category, amount, date } = req.body;
...
const getExpense = async (req, res) => {
  ...
  try {
    let expenses = await Expense.find({ user: req.user }).sort({ date: -1 });
    let filteredexpense = [];
    if (type === "all") {
    ...
    filteredexpense = expenses.filter((e) => {
...
const getExpenseDateBased = async (req, res) => {
